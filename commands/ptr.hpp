...
std::string ptrtoint(std::string source_name, std::string destination_name) {
  return "%" + destination_name + " = ptrtoint i8* %" + source_name +
         " to i64\n";
}

std::string ptrtoint(int source_name, int destination_name) {
  return ptrtoint(std::to_string(source_name),
